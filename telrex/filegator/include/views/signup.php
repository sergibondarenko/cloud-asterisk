...
<form action="<?php echo $_SERVER['PHP_SELF'].'?signup=1'; ?>" method="post">
  <div class="line"><label for="username"><?php echo lang::get("Username")?> *: </label><input type="text" name="username" id="username" value="<?if (isset($_POST['username'])) echo $_POST['username']?>"/></div>
  <div class="line"><label for="email"><?php echo lang::get("Email")?> *: </label><input type="text" name="email" id="email" value="<?if (isset($_POST['email'])) echo $_POST['email']?>"/></div>
